...
const CreateKMReading = ({ isOpen, onOpenChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.code === 201 || res?.code === 200) {
        toast.success(res?.message || "KM Reading created successfully");
        setFormData({
          km_readings_date: new Date().toISOString().split("T")[0],
          user_id: "",
          km_readings: "",
        ...
        toast.error(res?.message || "Failed to create KM Reading");
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Add KM Reading</DialogTitle>
        </DialogHeader>

        <form
          id="create-km-reading-form"
...
export default CreateKMReading;
